 ...
 */
/*
    MediaPlayer를 이용해서 음악을 재생하고 해당 곡의 재생이 끝났다는 것을 아는 방법은?
    -
 */
...
public class MusicService extends Service implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
    ...
    //뒤로 감는 기능
    public void rewMusic(){//rewind
        //만일 음악이 준비되지 않았다면
        if(!isPrepared) return; //메소드를 여기서 끝내라
        //현재 재생 위치보다 뒤로 10초 이동
    ...
    //앞으로 감는 기능
    public void ffMusic(){ //fast-foward
        //만일 음악이 준비되지 않았다면
        if(!isPrepared) return; //메소드를 여기서 끝내라
        //현재 재생 위치보다 앞으로 10초 이동
        int current = mp.getCurrentPosition();
    ...
    //다음 곡으로 자동으로 이동했는지 감시할 리스너 인터페이스
    public interface OnMoveToListener{
        public void moved(int index);
    }

    //어디에선가 (Activity) 바인딩(연결)이 해제되었을 때 호출되는 메소드
    @Override
    public boolean onUnbind(Intent intent) {
        //OnMoveToListenr를 제거한다,
        listener = null;
        return super.onUnbind(intent);
    }

    public void setOnMoveToListener(MainActivity mainActivity) {
        //Activity의 참조값이 OnMoveToListener type으로 전달되는 메소드
    ...
    public void makeManualCancelNoti() {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, AppConstants.CHANNEL_ID)
                ...
                .setContentTitle(musicList.get(currentIndex).getTitle()) //알림의 제목
                .setContentText(info)
