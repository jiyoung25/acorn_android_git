...
public class MainActivity extends AppCompatActivity  implements AdapterView.OnItemClickListener, MusicService.OnMoveToListener {
    ...
    ServiceConnection sConn=new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            ...
            //service에 재생할 음악 목록을 전달한다.
            //재생 음악 목록을 service에도 전달을 해준다.
            service.setMusicList(musicList);
        }
    ...
    protected void onStart() {
        ...
        intent.setAction("Dummy Action"); //실제로 하는 것은 없으나, 이것을 하지 않으면 MusicService의 startcommand에서 오류가 난다.(nullpointexception방지용)
        //서비스 시작 시키기
        //이미 서비스가 동작 중이라면 onStartCommand()메소드만 다시 시작된다 (no problem이라는 소리)
        
        //startService(intent);
        // 액티비티의 bindService() 메소드를 이용해서 연결한다.
        // 만일 서비스가 시작이 되지 않았으면 서비스 객체를 생성해서
        // 시작할 준비만 된 서비스에 바인딩이 된다.
        //이것의 문제점은 바인드 서비스 한 것은 unbind하면 back했을 때 바로 정지된다.
    ...
    protected void onStop() {
        ...
        if(isConnected){
            //서비스 바인딩 해제
            //바인드 서비스 한 것은 unbind하면 back했을 때 바로 정지된다.
            unbindService(sConn);
            isConnected=false;
        }
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        //click한 cell의 index를 service에 전달하여 해당 음악을 재생하도록 한다.
    ...
    class MusicListTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String jsonStr) {
            ...
            try {
                ...
                //model의 data가 바뀌었다고 adaptor에 알려서 ListView가 update되도록 한다.
